...
import 'package:portfolio_web/Shared/animations/animated_cursor.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  late List<double> _sectionScrollOffsets = [];
  ...
  void _calculateSectionOffsets() {
    ...
    final RenderBox? scrollableRenderBox = _scrollableKey.currentContext
    ...
    if (scrollableRenderBox == null || !scrollableRenderBox.hasSize) {
      ...
      []; // Clear or keep previous if partial layout is possible
      return;
    }

    _sectionScrollOffsets = _sectionKeys.map((key) {
      final RenderBox? sectionRenderBox = key.currentContext
          ?.findRenderObject() as RenderBox?;
      ...
      final Offset offsetInScrollable = sectionRenderBox.localToGlobal(
          Offset.zero, ancestor: scrollableRenderBox);
  ...
  void _onScroll() {
    ...
    final RenderBox? homeRenderBox = _homeSectionKey.currentContext
        ?.findRenderObject() as RenderBox?;
    if (homeRenderBox != null) {
      ...
      bool shouldShow = scrollPosition >
          (homeSectionHeight * 0.75); // Adjust 0.75 as needed
    ...
    for (int i = 0; i < _sectionKeys.length; i++) {
      ...
      final RenderBox? renderBox = _sectionKeys[i].currentContext
          ?.findRenderObject() as RenderBox?;
      ...
      if (renderBox != null) {
        ...
        if (scrollPosition >= sectionStartOffset -
            (viewportHeight * 0.2) && // Section starts appearing
            scrollPosition < sectionEndOffset -
                (viewportHeight * 0.2)) { // Section is still mostly visible
